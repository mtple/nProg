...
// Cache upstream responses for 60s. The upstream /transfers endpoint is
...
export async function GET(req: NextRequest) {
  ...
  const allowed = ["page", "limit", "chainId", "collector", "content_type", "artist", "type"];
  for (const key of allowed) {
    const value = incoming.get(key);
    if (value) params.set(key, value);
  }

  const url = `${API_BASE}/transfers?${params.toString()}`;

  const res = await fetch(url, {
    next: { revalidate: REVALIDATE_SECONDS },
  });

  const data = await res.json().catch(() => ({ status: "error", message: "Transfers fetch failed" }));
